...
def inject_skills(git_root: Optional[Path]) -> bool:
    ...
    try:
        ...
        for kind in ("agents", "prompts"):
            ...
            # Also exclude ``agdt.README.md`` — the managed manifest file
            # generated by a previous run. In editable-install scenarios the
            # repo's own ``.github/<kind>`` serves as source, so a leftover
            # manifest would otherwise be picked up as an injectable skill.
            source_files = [
                p
                for p in source_files
                if p.relative_to(source_dir) != Path("README.md")
                and p.name != "agdt.README.md"
            ]
