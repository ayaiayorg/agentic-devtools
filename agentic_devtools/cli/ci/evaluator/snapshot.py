...
def _get_latest_agent_comment(provider: CIPlatformProvider, pr_number: int) -> CommentInfo | None:
...
def _has_evaluator_sentinel_comment(
    ...
    try:
        ...
        logger.warning("Failed to fetch issue comments for evaluator sentinel check on PR #%d", pr_number)
        return False
    ...
    *,
    ...
    if review_id:
        try:
            ...
            logger.warning("Failed to fetch review comments for review %d", review_id)

    # 5. Find latest Copilot agent comment (issue comment, not review comment)
    latest_agent_comment: CommentInfo | None = None
    try:
        latest_agent_comment = _get_latest_agent_comment(provider, pr_number)
    except Exception:
