...
def stage_changes(dry_run: bool) -> None:
    ...
    On branches that do **not** end with ``-agdt``, each entry from
    ``AGDT_GITIGNORE_ENTRIES`` is also unstaged (as ``.agdt/{entry}``) to
    prevent runtime state from being accidentally committed to code branches.
    ...
    if dry_run:
        ...
        if not branch.endswith("-agdt"):
            ...
            for entry in AGDT_GITIGNORE_ENTRIES:
                print(f"[DRY RUN] .agdt/{entry} will stay staged (on -agdt branch)")
    ...
    if not branch.endswith("-agdt"):
        ...
        try:
            ...
            if toplevel.returncode == 0 and toplevel.stdout.strip():
                gi_path = Path(toplevel.stdout.strip()) / ".agdt" / ".gitignore"
            else:
                gi_path = Path(".agdt") / ".gitignore"
            if gi_path.is_file():
                gi_path.unlink()
