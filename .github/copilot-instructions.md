...
- **Single Global Install**: Install once with `uv pip install --system agentic-devtools` (preferred) or `pip install agentic-devtools` or `pipx install agentic-devtools`; all worktrees use the same installation
...
Numeric issue IDs (e.g., `42`) are JSON-parsed as integers by `agdt-set`.
Read paths that resolve the current issue key normalize that value to its
string representation, so `agdt-set issue_key 42` and `agdt-set issue_key "42"`
...
| `agdt-request-changes-with-suggestion` | Request changes with structured code suggestions | pull_request_id, file_review.file_path, file_review.summary, file_review.suggestions (each suggestion must include `replacement_code`) OR `--file-path`, `--summary`, `--suggestions`, `--pull-request-id` |
...
### Commit Hash Source of Truth

The PR commit hash is stored in multiple locations, each serving a distinct purpose.
