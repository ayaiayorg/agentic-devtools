...
```bash
# View commits that touched this file on our branch (since diverging from main)
git log --oneline main..HEAD -- path/to/file

# View commits that touched this file on their branch
git log --oneline HEAD..main -- path/to/file

# View detailed commit messages for our changes
git log -p main..HEAD -- path/to/file

# View detailed commit messages for their changes
git log -p HEAD..main -- path/to/file
```
...
# Preferred: Use agentic-devtools for test execution
agdt-test                    # Runs full test suite (background task)
agdt-task-wait               # Wait for test results

# Fallback: Direct command if agdt-test not available
...
# Preferred: Use agentic-devtools (runs in background with proper state management)
...
`_version.py` is gitignored and should never appear in merge conflicts. If it does, discard it:

```bash
git checkout --theirs agentic_devtools/_version.py
# Or simply remove and let hatch-vcs regenerate:
git rm --cached agentic_devtools/_version.py 2>/dev/null
```
