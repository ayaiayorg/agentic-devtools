...
#
# Internal helper: removes trailing model-attribution footer lines from text.
# Only strips when the last non-empty line matches the footer pattern, so
# markdown horizontal rules (---) elsewhere in the content are preserved.
# ---------------------------------------------------------------------------
_strip_footer_from_text() {
    local text="$1"
    # Match footer pattern: optional blank lines, ---, *Generated by Copilot SDK (...)*
    # at end of text. Use grep -c to check, then sed to remove only trailing footer.
    if printf '%s' "$text" | grep -qE '^\*Generated by Copilot SDK \(.*\)\*$'; then
        # Remove trailing: blank line(s) + --- + attribution line + optional trailing newline
