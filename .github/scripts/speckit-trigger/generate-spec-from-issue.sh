...
# ---------------------------------------------------------------------------
# call_llm <prompt_text> [max_tokens]
#
...
call_llm() {
    local prompt="$1"
    local max_tokens="${2:-32768}"
    export COPILOT_MAX_TOKENS="$max_tokens"
...
run_analyze_phase() {
    ...
    spec_content=$(cat "$SPEC_DIR/spec.md")
    plan_content=$(cat "$SPEC_DIR/plan.md")
    tasks_content=$(cat "$SPEC_DIR/tasks.md")
