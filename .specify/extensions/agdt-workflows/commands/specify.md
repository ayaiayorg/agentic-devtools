...
handoffs: 
  ...
  - label: Build Technical Plan
    ...
    agent: speckit.agdt:plan
    prompt: Create a plan for the spec. I am building with...

  - label: Clarify Spec Requirements

    agent: speckit.agdt:clarify
...
The text the user typed after `/speckit.agdt:specify` in the triggering message
...
2. **Determine the GitHub issue number** for this feature:
   ...
   b. Identify the GitHub issue number:
      ...
      - If the user mentioned an issue number (e.g., "#42" or "issue 42"),
        use that number directly.
      - If the feature originated from a GitHub issue, find its number.
      - If no issue exists yet, create one first or ask the user for the
        issue number.
      - The issue number is required — it links the spec to its originating
        GitHub issue.

   c. Run the script `{SCRIPT}` with the issue number and short-name:

      - Pass `--issue <ISSUE_NUMBER>` and
        `--short-name "your-short-name"` along with the feature description
      - Bash example: `{SCRIPT} --json --issue 42 --short-name "user-auth"
   ...
   **IMPORTANT**:
   ...
   - The `--issue` number must be a valid GitHub issue number (positive
     integer)
...
6. **Specification Quality Validation**: After writing the initial spec,
   ...
   structure with these validation items:
      ...
      **Feature**: [Link to spec.md]
      ...
      - Items marked incomplete require spec updates before `/speckit.agdt:clarify` or
        `/speckit.agdt:plan`
      ```
...
7. Report completion with branch name, spec file path, checklist results, and
   readiness for the next phase (`/speckit.agdt:clarify` or `/speckit.agdt:plan`).
