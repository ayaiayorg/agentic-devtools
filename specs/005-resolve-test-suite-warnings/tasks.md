...
- [ ] T029 Commit all changes using `agdt-git-save-work` with a conventional commit message of the form `fix([#958](https://github.com/ayaiayorg/agentic-devtools/issues/958)): resolve test suite warnings with filterwarnings=error` and a matching footer line `[#958](https://github.com/ayaiayorg/agentic-devtools/issues/958)`, including the tarfile fix, pytest.warns conversions, autopilot=False updates, and filterwarnings config in the body; run `agdt-task-wait` to confirm the push succeeds
