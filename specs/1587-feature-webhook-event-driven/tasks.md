...
| Task Phase | Plan Phase |
| --- | --- |
| Phase 1: Setup | Phase 1: Core Monitor Workflow |
| Phase 2: Foundational — Cache & Job Structure | Phase 1: Core Monitor Workflow |
| Phase 3: User Story 1 — Immediate Post-Agent PR Processing (P1) | Phase 1: Core Monitor Workflow |
| Phase 4: User Story 2 — Idempotent Event Processing (P1) | Phase 1: Core Monitor Workflow |
| Phase 5: User Story 3 — Graceful Coexistence with Existing Triggers (P2) | Phase 2: Guard Checks |
| Phase 6: User Story 4 — Monitor Workflow Observability (P3) | Phase 3: Observability & Logging |
| Phase 7: Testing & Validation | Phase 4: Testing & Validation |
| Phase 8: Polish & Cross-Cutting | Phase 4: Testing & Validation |
...
- [ ] T030 [US1] Dry-run test for FR-001 — verify monitor generates a dispatch command for each new terminal event (success path) (FR-001)
- [ ] T031 [US1] Dry-run test for FR-003 — verify `--field pr_number` is correctly set in the generated dispatch command (success path) (FR-003)
- [ ] T032 [US1] Dry-run test for FR-005 — verify `--field trigger_reason=agent_session_finished` is correctly set in the generated dispatch command (success path) (FR-005)
- [ ] T033 [US1] Dry-run test for FR-007 — verify both `copilot_work_finished` and `copilot_work_finished_failure` events trigger dispatch (success path) (FR-007)
...
- [ ] T029 Final review — verify all FRs are addressed and all implementation checklist items are complete
...
T019 → T020
T013 → T021 → T022, T030, T031, T032, T033 (parallel)
T022 → T023 → T024 → T025
