...
> **⚠️ PR description note**: The PR description was auto-generated by the SpecKit GitHub Action and may list links to
> optional artifacts (`research.md`, `data-model.md`, `quickstart.md`, `contracts/`) that are gitignored and
> **intentionally not committed**. The tracked-artifacts line above is the authoritative list of what reviewers can open.
...
| LangGraph | Currently a core dependency (`>=0.2.0`) used by existing orchestration modules; in the next SpecKit phase (tasks), migrate imports behind optional-dependency-safe boundaries, then align to `[langchain]` extra with target `>=0.4,<1.0` |
| LangGraph checkpoint | `langgraph-checkpoint-sqlite>=3.0.1` (currently core dependency; placement to be reassessed when `langgraph` moves behind the `[langchain]` extra) |
...
- CLI selection standardizes on `--engine langchain` per the clarified
  ...
  A `--use-langchain` flag will be supported as a deprecated alias
  that maps to `--engine langchain` for backward compatibility with
  Issue #1428's acceptance criteria
...
2. **State compatibility**: LangChain path writes backward-compatible `review-state.json` with an additive optional `engine` field on session entries
...
└──────────────────────────────┬──────────────────────────────────┘
              ...
              │ (unchanged)   │  │ → import langchain_core  │
              └───────────────┘  │ → validate config        │
                                 └───────────┬─────────────┘
                                             │
                                 ┌───────────▼─────────────┐
...
| 2.2 | `agentic_devtools/orchestration/review/preflight.py` (new) | Verify `import langchain_core` succeeds (PyPI package `langchain-core`), check config present |
| 2.3 | `pyproject.toml` | Add `[langchain]` optional extra with `langchain-core>=0.3,<1.0` and `langgraph>=0.4,<1.0`; explicitly remove `langgraph` from core dependencies and adjust `langgraph-checkpoint-sqlite` placement/guards as needed so default install remains valid |
...
| `langgraph-checkpoint-sqlite` | `>=3.0.1` | Currently core dependency; placement to be reassessed in Phase 2 (task 2.3) when `langgraph` moves to `[langchain]` extra |
