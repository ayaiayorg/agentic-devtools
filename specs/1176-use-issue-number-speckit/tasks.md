...
- [ ] T011 [US2] Add edge-case test for 3-digit issue-number overlap with legacy namespace:
...
- [ ] T012 [US2] Add edge-case test for 3-digit issue-number safe reuse:
...
- [ ] T013 [US2] Implement collision detection in `.github/scripts/speckit-trigger/generate-spec-from-issue.sh` —
  add glob scan for `${ISSUE_NUMBER}-*` (raw, unpadded) after `BRANCH_NAME` is computed,
  before the `SPEC_DIR` assignment
- [ ] T014 [US2] Implement Source Issue verification guard for 3-digit issue numbers (FR-015):
...
- [ ] T015 [US2] Add directory reuse logic: when `EXISTING_DIR` is found, set `SPEC_DIR` and `BRANCH_NAME`
  from existing dir; otherwise create new `${ISSUE_NUMBER}-${SHORT_NAME}` directory (raw, unpadded issue number)
- [ ] T016 [US2] Ensure `SPEC_FILE` output variable also reflects the reused directory path (verify `SPEC_FILE` is derived from `SPEC_DIR`)
- [ ] T017 [US2] Run collision detection test cases from T008–T012 to confirm all pass
...
- [ ] T018 [US3] Fix autoincrement regex in `get_next_feature_number` in
...
- [ ] T019 [US3] Add test case: specs dir has `001-foo`, `002-bar`, `42-issue` → `get_next_feature_number` returns 3 (ignores `42-issue`)

### 5b: `create-new-feature.sh` — spec directory scan

- [ ] T020 [P] [US3] Fix autoincrement regex in `get_highest_from_specs` in
  `.specify/scripts/bash/create-new-feature.sh` (inside the spec directory scan loop):
  change `grep -o '^[0-9]\+'` to filter `^[0-9]\{3\}-` with continue, then extract `^[0-9]\{3\}` with `10#$number` base-10 conversion
- [ ] T021 [P] [US3] Verify `get_highest_from_branches` in `.specify/scripts/bash/create-new-feature.sh`
  already correctly uses `^[0-9]\{3\}-` — no change needed, document as confirmed

### 5c: `create-new-feature.ps1` — both functions

- [ ] T022 [P] [US3] Fix `Get-HighestNumberFromSpecs` regex in `.specify/scripts/powershell/create-new-feature.ps1` (in the `Get-HighestNumberFromSpecs` function): change `'^(\d+)'` to `'^(\d{3})-'`
- [ ] T023 [P] [US3] Fix `Get-HighestNumberFromBranches` regex in `.specify/scripts/powershell/create-new-feature.ps1` (in the `Get-HighestNumberFromBranches` function): change `'^(\d+)-'` to `'^(\d{3})-'`

### 5d: Verification

- [ ] T024 [US3] Run autoincrement filter test cases from T003, T004, T019 to confirm all three scripts correctly ignore non-3-digit prefixes
- [ ] T025 [US3] Manual verification: confirm existing `specs/` directory contents (all 3-digit prefixed) still produce correct next number
...
- [ ] T026 [P] Update `specs/README.md` — add section documenting issue-numbered directories (`<ISSUE_NUMBER>-<slug>` from raw issue number, unpadded)
  vs autoincrement directories, and that autoincrement ignores issue-numbered dirs
- [ ] T027 [P] Update `SPEC_DRIVEN_DEVELOPMENT.md` — document the CI pipeline directory naming convention
  for issue-driven directories (`<ISSUE_NUMBER>-<slug>` using the raw unpadded issue number)
  and ISSUE_NUMBER validation rules
- [ ] T028 [P] Add inline comments in `.github/scripts/speckit-trigger/generate-spec-from-issue.sh` referencing FR-011, FR-004, FR-007, FR-012 at each modification site

### Integration testing

- [ ] T029 Run full manual test matrix from plan Phase 4a (8 test cases) covering all three scripts
- [ ] T030 Verify `check-idempotency.sh` continues to work correctly with issue-numbered directories (no changes needed to that script, but verify no regressions)
- [ ] T031 Verify the existing `specs/1176-use-issue-number-speckit/` directory is correctly ignored by autoincrement in all three scripts (real-world validation with actual repo contents)

### Workflow idempotency

- [ ] T032 [P] Update `speckit-issue-trigger.yml` "Commit Planning Artifacts" step to handle existing
...
- [ ] T033 [P] Update `speckit-issue-trigger.yml` "Push Branch" step to use `git push --force-with-lease`
  (or equivalent) so reruns can update the remote branch instead of failing with a non-fast-forward error
- [ ] T034 [P] Add test case to `test-autoincrement-filter.sh` for workflow rerun idempotency:
...
- [ ] T035 [P] Update `check_feature_branch` in `.specify/scripts/bash/common.sh` to accept both
...
- [ ] T036 [P] Update `find_feature_dir_by_prefix` in `.specify/scripts/bash/common.sh` to extract
...
- [ ] T037 Ensure `test-autoincrement-filter.sh` is executable (`chmod +x`) and has a shebang line
- [ ] T038 Final review: confirm no `grep -o '^[0-9]\+'` patterns remain in any of the three modified scripts
...
T001 ─→ T002 ─→ T003, T004 (parallel)
    ...
    T008, T009, T010, T011, T012 ─→ T013 ─→ T014 ─→ T015 ─→ T016 ─→ T017
                                                       │
                              T018 ─→ T019 ─┤
                              T020 ──────────┤  (parallel with T022, T023)
                              T022 ──────────┤
                              T023 ──────────┤
                                             ↓
                                     T024 ─→ T025
                                             │
                              T026, T027, T028 (parallel)
                              T032, T033, T034 (parallel — workflow idempotency)
                              T035, T036 (parallel — .specify helper compat)
                                             │
                                     T029 ─→ T030 ─→ T031
                                             │
                                     T037 ─→ T038
