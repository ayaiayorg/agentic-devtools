...
**Feature Branch**: `speckit/1505/phase-1-specify`
**Created**: 2026-05-21
**Status**: Draft
...
- Extraction of shared validation helpers usable by both Phase 1 and Phase 2
...
3. **Given** Phase 1 generates a `spec.md` that is under 2048 bytes in size, **When** the output is validated, **Then** validation fails with feedback indicating the spec is below the minimum content
   threshold.
...
1. **Given** Phase 1 validation fails on the initial LLM output, **When** the retry mechanism fires, **Then** the LLM receives a re-prompt containing categorized feedback (MISSING_SECTIONS,
...
3. **Given** the retry limit (3 attempts) is exhausted without producing a valid spec, **When** all retries fail, **Then** the pipeline exits with a deterministic error code and an actionable error
   message listing all validation failures from the final attempt.

4. **Given** a retry attempt encounters an operational failure (e.g., LLM API timeout), **When** the error is detected, **Then** it does NOT count against the retry limit (consistent with Phase 2
   behavior where return code 2 means operational failure).
...
As a SpecKit maintainer, I want the section-counting and structural validation logic to be extracted into shared helper functions usable by both Phase 1 and Phase 2, so that validation rules are
defined once and enforced consistently across phases.
...
1. **Given** a new shared helper `validate_spec_quality` is created, **When** called with a spec file path, **Then** it checks all Phase 1 quality rules (mandatory sections, minimum counts, size
   threshold, measurability, bullet-summary detection) and returns structured failure reasons.
...
3. **Given** the shared helpers are defined in a library file, **When** both `run_specify_phase` and `run_clarify_phase` source them, **Then** no code duplication exists for section extraction and
   counting logic.
...
As a SpecKit maintainer, I want validation thresholds (minimum FR count, minimum user story count, minimum byte size, retry limit) to be defined as named constants at the top of the validation script,
...
- **What happens when the LLM produces an empty or whitespace-only response?** The existing empty-content check (already present in Phase 1) fires before validation, failing the phase immediately with
...
- **How does the system handle a spec that passes structural validation but has fewer than the minimum acceptance scenarios per user story?** The validator checks for the presence of Given/When/Then
  patterns within the User Scenarios section. If fewer than 3 user stories have at least one acceptance scenario each, validation fails.

- **What happens if `strip_llm_preamble` removes content that causes the spec to drop below thresholds?** Validation runs AFTER preamble stripping and heading normalization, so the validated content
...
- **FR-004**: The system MUST require a minimum of 3 user stories with acceptance scenarios (sections matching `### User Story N` pattern containing at least one `**Given**`/`**When**`/`**Then**`
  sequence each).
...
- **FR-006**: The system MUST reject specs below a minimum content threshold of 2048 bytes (after preamble stripping).
...
- **FR-009**: The structured feedback provided to the LLM on retry MUST be categorized (e.g., MISSING_SECTIONS, INSUFFICIENT_REQUIREMENTS, INSUFFICIENT_USER_STORIES, BELOW_SIZE_THRESHOLD,
  NON_MEASURABLE_CRITERIA, BULLET_SUMMARY_DETECTED) with specific counts and thresholds.

- **FR-010**: Operational failures (LLM API errors, timeouts) MUST NOT count against the retry limit, consistent with the Phase 2 clarify retry contract (return code 2 = operational failure).

- **FR-011**: When all retries are exhausted, the system MUST exit with a non-zero exit code and print all validation failures from the final attempt to stderr.
...
- **FR-013**: Shared helper functions (section extraction, requirement counting, user story counting) MUST be extracted into a library file sourceable by both Phase 1 and Phase 2 code paths.
...
- **NFR-002**: All validation failure messages MUST be human-readable and actionable, clearly stating what failed, what the threshold is, and what the actual value was.

- **NFR-003**: The retry mechanism MUST produce deterministic outcomes — given the same LLM output, the validation result MUST be identical across runs.

- **NFR-004**: Threshold constants MUST be defined as named variables at the top of the validation module, not as inline magic numbers.
...
- **NFR-006**: New validation logic MUST be covered by shell tests in `test_content_preservation.sh` or a dedicated test file, achieving 100% branch coverage of the validation function.

### Key Entities

- **Spec Quality Validator**: A shell function (`validate_spec_quality`) that performs absolute-threshold validation on a freshly generated `spec.md`. Unlike `validate_structural_integrity` (which
  compares candidate to original), this validator checks minimum quality standards for a new spec with no prior version.
...
- **Structured Retry Feedback**: A categorized, machine-readable feedback string generated when validation fails. Includes category labels, actual vs. expected counts, and remediation instructions.
  Fed directly into the LLM re-prompt.

- **Retry Budget**: A bounded counter (default: 3) tracking validation-failure retries. Operational failures do not decrement the budget. Exhaustion triggers deterministic pipeline failure.

- **Quality Thresholds**: Named constants controlling validation sensitivity: `MIN_FUNCTIONAL_REQUIREMENTS` (5), `MIN_USER_STORIES` (3), `MIN_SPEC_BYTES` (2048), `MIN_MEASURABLE_CRITERIA_PCT` (50),
...
- **SC-006**: The new validation function has 100% branch coverage via dedicated shell tests, with at least 10 test cases covering the documented failure modes.
