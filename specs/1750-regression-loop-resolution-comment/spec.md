...
3. **Given** a review comment being resolved where no `TierResult` is available (edge case),
   **When** the resolution reply is posted, **Then** the system falls back to a reasonable
   default that still includes the HEAD commit link if available.
...
1. **Given** a resolution context where the HEAD commit SHA is known, **When** the reply is
   formatted, **Then** the reply body includes a `**HEAD**:` line with a markdown hyperlink to
...
- What happens when `tier_result` is `None` but the verdict is `COMMENT_RESOLVE`? The system
  should still post a reply — using a sensible default that includes the HEAD commit link if
  available, without crashing.
- How does the system handle a `tier_result` with an empty or `None` explanation field? The
  formatter should produce a valid reply with a placeholder or omit the rationale line
  gracefully.
- What happens when the HEAD commit SHA is available but the repository URL cannot be
  determined? The commit link should be omitted, and the rest of the reply should still be
  formatted correctly.
- How does the existing "has_existing_addressed_reply" detection interact with the new format?
  ...
  (via the HTML marker prefix) as valid existing replies to avoid duplicate posting.
...
- **FR-002**: The system MUST include a HEAD commit link in resolution replies when the commit
  SHA is available in the resolution context. The link format MUST be
...
- **FR-003**: The system MUST gracefully handle the case where `tier_result` is `None` by
  falling back to a reply that still includes the HEAD commit link (if available) and an
  appropriate static message indicating no detailed evaluation was performed.
...
- **NFR-002**: All new and modified code paths MUST have 100% branch coverage in unit tests,
  consistent with the repository's existing coverage policy.
...
- **NFR-003**: The reply format MUST remain stable across minor version bumps — the HTML marker
  ...
  notification parsers, dashboard scrapers) may depend upon.
...
- **ResolutionReply**: A structured object containing `html_marker`, `human_text`, and
  `model_id` — the decomposed parts of a formatted reply.
