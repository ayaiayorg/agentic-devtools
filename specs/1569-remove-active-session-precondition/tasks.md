...
- [ ] T002 Read the existing test suite in `tests/unit/cli/ci/pipeline/actions/resolve_threads/test_resolvethreadsaction.py`
- [ ] T003 Read FR-005, US2 scenario 1, and US3 scenario 2 in `specs/1559-refactor-loop-into-idempotent/spec.md`

## Phase 2: Foundational

- [ ] T004 Verify baseline tests pass by running `agdt-test-pattern tests/unit/cli/ci/pipeline/actions/resolve_threads/ -v`
...
- [ ] T013 [US2] Verify all remaining-precondition tests pass by running `agdt-test-pattern tests/unit/cli/ci/pipeline/actions/resolve_threads/ -v`
...
- [ ] T020 Run full test suite with `agdt-test` and `agdt-task-wait` to verify no regressions
- [ ] T021 Run `bash scripts/run-pr-checks.sh` to verify all PR checks pass (lint, format, markdownlint, mypy)
- [ ] T022 Verify squash and dispatch-repair test files are NOT modified by inspecting `git diff --name-only`
