...
Add validation to the SpecKit pipeline so checklist-oriented markdown files are required to contain real markdown checkbox items rather than only descriptive prose. The validator must distinguish between files with no checklist items and files with too few checklist items, ignore checkbox-like syntax inside fenced code blocks, report actionable results, and support both pipeline enforcement and standalone CLI usage. An optional remediation mode may re-prompt the LLM to regenerate deficient files using a bounded retry strategy.

## User stories

1. **P1 — Detection:** As a maintainer, I want the pipeline to detect checklist files that contain prose but no actual checkbox items so that incomplete generated artifacts are caught automatically.
2. **P1 — Analysis reporting:** As a developer, I want validation output to show which files failed, how many checkbox items were found, and why they were classified as deficient so that I can fix them quickly.
3. **P2 — Pipeline blocking:** As a CI owner, I want checklist files with fewer than 3 checkbox items to fail the pipeline so that obviously unusable checklist outputs do not get merged.
4. **P3 — LLM re-prompting:** As an AI-assisted workflow user, I want the system to optionally re-prompt the LLM when a generated checklist is invalid so that recoverable generation failures can be fixed automatically.
...
1. The system shall validate checklist-designated markdown files produced or consumed by the SpecKit pipeline.
2. The validator shall count markdown checkbox items using standard unchecked or checked task-list syntax, including lines beginning with `- [ ]`, `- [x]`, `* [ ]`, or `* [x]`.
3. The validator shall ignore checkbox-like syntax that appears inside fenced code blocks delimited by triple backticks.
4. The validator shall classify a file with **0** counted checkbox items as **prose-only**.
5. The validator shall classify a file with **1 or 2** counted checkbox items as **deficient**.
6. The validator shall classify a file with **3 or more** counted checkbox items as **valid**.
7. The validator shall assign **MEDIUM** severity to prose-only files.
8. The validator shall assign **LOW** severity to deficient files containing 1 or 2 checkbox items.
9. The pipeline shall fail when at least one validated file has fewer than 3 counted checkbox items (prose-only or deficient).
10. The pipeline shall distinguish prose-only failures (MEDIUM severity) from deficient failures (LOW severity) in its output.
11. The validation result shall include, for each file, the file path, checkbox count, classification, severity, and a human-readable explanation.
12. The validator shall support processing multiple files in a single run and shall report aggregate pass/fail status across all processed files.
13. The standalone CLI mode shall return a non-zero exit code when blocking validation failures are present.
14. The optional LLM re-prompting mode shall be disabled by default and enabled only through explicit configuration or invocation.
15. When LLM re-prompting is enabled, the system shall retry regeneration at most **2** times per invalid file and shall follow the staged remediation pattern used by sibling spec `#1191`.
...
4. Checked items such as `- [x]` shall count toward the threshold the same as unchecked items.
