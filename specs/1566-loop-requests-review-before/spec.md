...
- **Review Comment Thread**: A GitHub PR review comment thread (represented in the GitHub API as a review comment with replies). Each thread has an `isResolved` boolean state. Threads are identified
...
- **SC-004**: No increase in orchestrator run time beyond 2 seconds additional latency attributable to the thread-listing API call (measured as p95 latency delta in CI workflow telemetry over 50+
  runs).

- **SC-005**: All new code achieves 100% line coverage under the project's testing policy (`agdt-test-file` with `--source-file` targeting the modified orchestrator module must pass).
