...
See [research.md](research.md) for decisions on guard placement, return signature, and ADO payload mapping.
...
| `edited` trigger causes unwanted runs before guard code deploys | Medium | Low | Deploy code first (guard defaults to pass-through), then update YAML |
...
- **Deploy Order**: Python code (Phases 1-5) MUST be merged before YAML change (Phase 6) to avoid a window where `edited` events trigger without the guard
