...
4. **Given** a PR in draft state with changes pushed, **When** the loop runs, **Then** publish executes first, then subsequent actions evaluate against the now-published state.
...
report the action as "failed (transient)" and subsequent actions should continue evaluation. The next loop run will naturally retry the failed action.
...
- **FR-001**: The system MUST evaluate all 8 actions (guards, publish, request-review,
  ...
  event type. This means the same code path executes whether the trigger was `issue_comment`, `workflow_run(completed)`, or `pull_request_review(submitted)`.
...
- **FR-003**: The system MUST determine "active Copilot session" by comparing issue event IDs: a `copilot_work_started` event without a subsequent `copilot_work_finished` or
  `copilot_work_finished_failure` event (by ID ordering) indicates an active session. This single check replaces the entire squash-wait state machine.
...
- **FR-008**: Every run MUST post a summary comment on the PR containing a workflow run link, action evaluation table (all 8 actions with precondition results and outcomes), and a collapsed state
  snapshot section showing key decision inputs.
...
- **FR-011**: The system MUST gather a full PR state snapshot once at the beginning of each run and use it as the single external source for evaluations in that run. Later actions MAY evaluate
  against in-memory derived state produced by earlier actions in the same run (for example, publish changing draft → ready) without re-fetching PR state from external APIs.
...
- **FR-014**: The pipeline MUST exit early (after guards) if any hard-blocking guard
  ...
  guard failure reason clearly indicated.
...
- **NFR-003**: The pipeline MUST be resilient to individual action failures — a failure in action N MUST NOT prevent evaluation of actions N+1 through 8 (unless N is guards, which gates the entire
  pipeline). Failed actions are reported in the summary and retried on the next run.
...
- **PRStateSnapshot**: A frozen representation of all PR state gathered at run start — HEAD SHA, commit count above merge-base, CI status per check, review state (latest Copilot review ID, state,
  inline comment count), active session status, unresolved thread count and IDs, labels, draft status, mergeable status, and requested reviewers.
