...
- **FR-002a**: The `RequestReviewAction` MUST skip on cross-run re-triggers when a repair was dispatched in a prior run and the HEAD SHA has not changed since
  that dispatch. Detection mechanism: the existing deduplication marker comment (posted by `DispatchRepairAction`) contains the HEAD SHA at dispatch time; on re-trigger, the orchestrator reads this
  marker and compares against current HEAD. Suppression is lifted when HEAD SHA differs from the marker SHA (indicating the repair agent pushed new code).
