...
class TestBuildTree:
    ...
    def test_calls_update_index_per_entry(self, mock_run):
        """build_tree calls _run_plumbing for each entry in sorted order."""
    ...
    @patch("agentic_devtools.cli.git.agdt_branch._run_plumbing")
    def test_temp_index_is_nonexistent_path(self, mock_run):
        """build_tree points GIT_INDEX_FILE at a path that does not pre-exist."""
        mock_run.return_value = MagicMock(returncode=0, stdout="tree\n", stderr="")
        build_tree({"f.txt": "sha"})

        env = mock_run.call_args_list[0][1]["env"]
        index_path = env["GIT_INDEX_FILE"]
        # The index path should end with /index (inside a temp dir),
        # not be an empty file created by mkstemp.
