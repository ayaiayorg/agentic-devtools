...
class TestAddPullRequestCommentActualCall:
    ...
    def test_is_pull_request_approval_flag_ignored(
        ...
        """is_pull_request_approval state key has no special effect — comment is posted as a normal new thread."""
        mock_get_repo.return_value = "repo-guid-123"
        mock_req_module = MagicMock()
        ...
        # Setting this flag used to trigger special approval logic; now it is ignored
        state.set_value("is_pull_request_approval", True)

        azure_devops.add_pull_request_comment()

        # Should create a normal new thread endpoint
        post_call = mock_req_module.post.call_args_list[-1]
        body = post_call[1]["json"]
        assert "LGTM!" in body["comments"][0]["content"]
