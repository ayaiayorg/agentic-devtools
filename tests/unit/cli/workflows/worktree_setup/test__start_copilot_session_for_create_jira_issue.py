...
class TestStartCopilotSessionForCreateJiraIssue:
    ...
    def test_delegates_to_generic_helper(self, mock_state_dir, mock_generic, tmp_path):
        ...
        assert call_kwargs["start_prompt"] == COPILOT_SESSION_START_PROMPT_CREATE_JIRA_ISSUE
...
class TestCopilotSessionStartPromptCreateJiraIssue:
    """Tests for the COPILOT_SESSION_START_PROMPT_CREATE_JIRA_ISSUE constant."""
    ...
    def test_prompt_references_workflow_name(self):
        """The prompt must reference the create-jira-issue workflow via agent handoff."""
        assert "@agdt.create-jira-issue.initiate" in COPILOT_SESSION_START_PROMPT_CREATE_JIRA_ISSUE

    def test_prompt_does_not_contain_template_variables(self):
        """The prompt must be a static string with no template variables."""
        assert "{{" not in COPILOT_SESSION_START_PROMPT_CREATE_JIRA_ISSUE
        assert "}}" not in COPILOT_SESSION_START_PROMPT_CREATE_JIRA_ISSUE
