...
def _build_sdk_modules() -> tuple[MagicMock, MagicMock, MagicMock, MagicMock]:
...
def _build_sdk_client_for_response(response: str) -> tuple[MagicMock, MagicMock, MagicMock]:
    mock_session = MagicMock()
    mock_session.disconnect = AsyncMock()
...
class TestFinalizePostRepair:
    ...
    def test_verify_comments_via_sdk_happy_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        mock_copilot, mock_copilot_config, mock_session_module, mock_session = _build_sdk_modules()
        ...
        assert mock_copilot_config.SubprocessConfig.call_args.kwargs["github_token"] == "test-token"
    ...
    def test_run_prompt_via_sdk_reraises_runtime_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("COPILOT_GITHUB_TOKEN", "test-token")  # type: ignore[attr-defined]
        mock_copilot, mock_copilot_config, mock_session_module, _mock_session = _build_sdk_modules()
