"""Tests for agentic_devtools.cli.github.resolve_review_threads._unresolve_thread."""

import json
import subprocess
from unittest.mock import patch

from agentic_devtools.cli.github.resolve_review_threads import _unresolve_thread

_MODULE = "agentic_devtools.cli.github.resolve_review_threads"


class TestUnresolveThread:
    """Tests for _unresolve_thread."""

    @patch(f"{_MODULE}.run_safe")
    def test_successful_unresolution(self, mock_run):
        """Return True when mutation reports isResolved=false (thread is now open)."""
        mock_run.return_value = subprocess.CompletedProcess(
            args=[],
            returncode=0,
            stdout=json.dumps({"data": {"unresolveReviewThread": {"thread": {"id": "PRT_a", "isResolved": False}}}}),
            stderr="",
        )
        assert _unresolve_thread("PRT_a") is True
    ...
    def test_nonzero_exit_returns_false(self, mock_run):
        ...
        assert _unresolve_thread("PRT_a") is False
    ...
    @patch(f"{_MODULE}.run_safe")
    def test_mutation_returns_still_resolved(self, mock_run):
        """Return False when mutation reports isResolved=true (thread remains resolved)."""
        mock_run.return_value = subprocess.CompletedProcess(
            args=[],
            returncode=0,
            stdout=json.dumps({"data": {"unresolveReviewThread": {"thread": {"id": "PRT_a", "isResolved": True}}}}),
            stderr="",
        )
        assert _unresolve_thread("PRT_a") is False
